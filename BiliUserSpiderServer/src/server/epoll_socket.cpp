...
int EpollSocket::bind_on(unsigned int ip) {
    ...
    if (sockfd == -1) {
        LError("socket error:{}", strerror(errno));
        return -1;
    }
    int opt = 1;
    ...
    if (bind(sockfd, (struct sockaddr *) &my_addr, sizeof(struct sockaddr)) == -1) {
        LError("bind error:{}", strerror(errno));
        return -1;
    }
    if (listen(sockfd, _backlog) == -1) {
        LError("listen error:{}", strerror(errno));
        return -1;
    }
...
int EpollSocket::listen_on() {
    if (_bind_ips.empty()) {
        ...
        for (size_t i = 0; i < _bind_ips.size(); i++) {
            ...
            LInfo("bind for ip:{} success", _bind_ips[i].c_str());
        }
    }

    LInfo("start Server Socket on port : {}", _port);
    return 0;
}
...
int EpollSocket::accept_socket(int sockfd, std::string &client_ip) {
    ...
    if ((new_fd = accept(sockfd, (struct sockaddr *) &their_addr, &sin_size)) == -1) {
        LError("accept error:{}", strerror(errno));
        return -1;
    }

    client_ip = inet_ntoa(their_addr.sin_addr);
    LDebug("server: got connection from {}\n", client_ip.c_str());
    return new_fd;
}
...
int EpollSocket::handle_accept_event(int &epollfd, epoll_event &event, EpollSocketWatcher &socket_handler) {
    ...
    LDebug("get accept socket which listen fd:{}, conn_sock_fd:{}", sockfd, conn_sock);
    ...
    if (epoll_ctl(_epollfd, EPOLL_CTL_ADD, conn_sock, &conn_sock_ev) == -1) {
        LError("epoll_ctl: conn_sock:{}", strerror(errno));
...
int EpollSocket::handle_readable_event(epoll_event &event) {
    ...
    if (epoll_context == NULL) {
        LWarn("Get context from read event fail!");
        return -1;
    }
...
int EpollSocket::handle_writeable_event(int &epollfd, epoll_event &event, EpollSocketWatcher &socket_handler) {
    EpollContext *epoll_context = (EpollContext *) event.data.ptr;
    if (epoll_context == NULL) {
        LWarn("Get epoll context fail in write event!");
        return -1;
    }
    int fd = epoll_context->fd;
    LDebug("start write data");
...
int EpollSocket::init_default_tp() {
    ...
    LInfo("thread pool not set, we will build for core size:{}", core_size);
...
int EpollSocket::add_listen_sock_to_epoll() {
    for (std::set<int>::iterator i = _listen_sockets.begin(); i != _listen_sockets.end(); i++) {
        ...
        if (epoll_ctl(_epollfd, EPOLL_CTL_ADD, sockfd, &ev) == -1) {
            LError("epoll_ctl: listen_sock:{}", strerror(errno));
            return -1;
        }
    }
    return 0;
...
int EpollSocket::multi_thread_handle_read_event(epoll_event &e) {
    ...
    LDebug("start handle readable event");
    ...
    if (ret != 0) {
        LWarn("add read task fail:{}, we will close connect.", ret);
        close_and_release(e);
        delete tdata;
        delete task;
    }
...
int EpollSocket::handle_event(epoll_event &e) {
    ...
    if (_listen_sockets.count(e.data.fd)) {
        if (_status != S_RUN) {
            LWarn("current status:{}, not accept new connect", _status);
            pthread_mutex_lock(&_client_lock);
            if (_clients == 0 && _status == S_REJECT_CONN) {
                _status = S_STOP;
                LInfo("client is empty and ready for stop server!");
            }
            pthread_mutex_unlock(&_client_lock);
        } 
        // accept connection
...
int EpollSocket::create_epoll() {
    ...
    if (_epollfd == -1) {
        LError("epoll_create:{}", strerror(errno));
        return -1;
    }
    return 0;
}
...
int EpollSocket::clear_idle_clients() {
    ...
    for (; it != _eclients.end(); it++) {
         ...
         if (ctx->_last_interact_time <= timeout_ts && 
             ...
             LDebug("find idle client fd:{}", ctx->fd);
             epoll_event e;
             memset(&e, 0, sizeof(e));
             e.data.ptr = ctx;
             remove_evs.push_back(e);
         } else {
             LDebug("find idle client but is reading, skip it!");
         }
    }
    pthread_mutex_unlock(&_client_lock);
    ...
    if (cnt > 0) {
        LInfo("find idle clients cnt:{}", cnt);
    }
    return cnt;
}
...
int EpollSocket::start_event_loop() {
    ...
    while (_status != S_STOP) {
        ...
        if (fds_num == -1) {
            ...
            LError("epoll_wait error:{}", strerror(errno));
            ret = -1;
            break;
        }
...
int EpollSocket::start_epoll() {
    ...
    int ret = init_tp();
    CHECK_RET(ret, "thread pool start error:{}", ret);

    ret = this->listen_on();
    CHECK_RET(ret, "listen err:{}", ret);

    ret = this->create_epoll();
    CHECK_RET(ret, "create epoll err:{}", ret);

    ret = this->add_listen_sock_to_epoll();
    CHECK_RET(ret , "add listen sock to epoll fail:{}", ret);
...
int EpollSocket::stop_epoll() {
    ...
    LInfo("stop epoll , current clients:{}", _clients);
    return 0;
}
...
int EpollSocket::close_and_release(epoll_event &epoll_event) {
    ...
    LDebug("connect close complete which fd:{}, ret:{}", fd, ret);
    return ret;
}
