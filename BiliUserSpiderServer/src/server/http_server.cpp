...
int HttpServer::start(int port, int backlog, int max_events) {
    LWarn("start() method is deprecated, please use start_sync() or start_async() instead!");
...
int HttpServer::start_async() {
    ...
    if (ret != 0) {
        LError("HttpServer::start_async err:{}", ret);
        return ret;
    }
    return 0;
}
...
int HttpServer::join() {
    if (_pid == 0) {
        LError("HttpServer not start async!");
        return -1;
    }
...
int HttpEpollWatcher::handle_request(Request &req, Response &res) {
    ...
    if (this->_resource_map->find(uri) == this->_resource_map->end()) { // not found
        ...
        //LInfo("page not found which uri:{}", uri.c_str());
        return 0;
    }
    ...
    if (method.get_names()->count(req._line.get_method()) == 0) {
        ...
        res._body.clear();
        LInfo("not allow method, allowed:{}, request method:{}", 
    ...
    LDebug("handle response success which code:{}, msg:{}", 
...
int HttpEpollWatcher::on_readable(int &epollfd, epoll_event &event) {
    ...
    LDebug("read success which read size:{}", read_size);
...
int HttpEpollWatcher::on_writeable(EpollContext &epoll_context) {
    ...
    LDebug("send complete which write_num:{}, read_size:{}", nwrite, read_size);

    if (ret == NEED_MORE_STATUS) {/* not send over*/
        LDebug("has big response, we will send part first and send other part later ...");
    ...
    if (ret == 0 && nwrite == read_size) {
        ...
        // 1. client_ip (with nginx proxy, not equal to real_ip)
        ...
        //LInfo("access_log {} {} status_code:{} cost_time:{} us, body_size:{}, client_ip:{}",
        //        http_method.c_str(), request_url.c_str(), res._code_msg.status_code,
        //        cost_time, res._body.size(), req.get_real_ip().c_str());
    }   
